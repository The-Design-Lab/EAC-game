    ...
    investments.forEach((invesment) => {
      ...
      switch (invesment) {
        case INVESTMENTS_VEHICLES.retirement:
          result[i][invesment.name] = quarterData[0][i];
          break;
        case INVESTMENTS_VEHICLES.SP:
          result[i][invesment.name] = quarterData[1][i];
          break;
        case INVESTMENTS_VEHICLES.CD:
          result[i][invesment.name] = quarterData[2][i];
          break;
        case INVESTMENTS_VEHICLES.inveesgo:
          result[i][invesment.name] = quarterData[3][i];
          break;
        case INVESTMENTS_VEHICLES.mutualFund:
          result[i][invesment.name] = quarterData[4][i];
          break;
        default:
          return;
      }
